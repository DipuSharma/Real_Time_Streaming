...
async def user_registration(payload=None, db=None):
    ...
    existed_data = db.query(UserTable).filter(UserTable.username == payload.username and UserTable.password == payload.password).first()
    if existed_data:
...
async def user_login(payload=None, db=None):
    try:
        data = db.query(UserTable).filter(UserTable.username == payload.username and UserTable.password == payload.password).first()
